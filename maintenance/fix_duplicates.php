<?php
/**
 * Cleanup Duplicate Questions Script (Root Version)
 * Fixes the issue where Total Max Score is doubled (e.g. 200 instead of 100)
 * Usage: Upload this file to your main ONET folder and run it.
 */
...
if (empty($duplicates)) {
    ...
    foreach ($duplicates as $dup) {
        $ids = explode(',', $dup['ids']);
        sort($ids); // Keep the first/lowest ID (original) or last?
        
        // Strategy: Keep the LAST one (Highest ID). 
        // Logic: Re-imports usually add new rows. The new rows might have correctly updated data (if changed).
        // Or if mistakenly imported same file twice, they are identical.
